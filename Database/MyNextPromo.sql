...
CREATE TABLE IF NOT EXISTS `MyNextPromoApp`.`accounts` (
  ...
  INDEX `fk_accounts_admin_users1_idx` (`admin_user_id` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `MyNextPromoApp`.`Education_levels`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `MyNextPromoApp`.`Education_levels` ;

CREATE TABLE IF NOT EXISTS `MyNextPromoApp`.`Education_levels` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `edu_level` VARCHAR(255) NULL,
  `value` INT NULL,
  `created_at` DATETIME NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS `MyNextPromoApp`.`Employees` (
  ...
  `position_id` INT NOT NULL,
  PRIMARY KEY (`id`, `Education_level_id`),
  INDEX `fk_Employees_users1_idx` (`account_id` ASC) VISIBLE,
  INDEX `fk_Employees_Education_levels1_idx` (`Education_level_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Employees_users1`
    ...
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Employees_Education_levels1`
    FOREIGN KEY (`Education_level_id`)
    REFERENCES `MyNextPromoApp`.`Education_levels` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
